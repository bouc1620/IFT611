...
private:
    // this peer's id
    usr_t self;
    // the address of the signed Int16 array on the heap used to exchange position vectors between
...
public:
    ...
    // Document constructor with peer id and the address of the signed Int16 array on the heap used
    // to exchange position vectors between JavaScript and WebAssembly
    ...
    // push the Character object at the specified index onto the heap and return the length of its
    // position vector
    ...
    // insert a Character in the document from local, also writes the Character's position vector
    // on the reserved space on the heap and returns its length
    int insert_fromLocal(chr_t chr, int idx);

    // deletes a Character from the document that was deleted from local and returns the length of
    // its position vector
    ...
    // returns the index of the character to remove in the Codemirror editor or -1 if the Character
    // was added to the deletion backlog
